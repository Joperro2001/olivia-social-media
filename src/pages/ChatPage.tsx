...
const ChatPage: React.FC = () => {
  ...
  return <div className="flex flex-col h-[100vh] bg-[#FDF5EF] pb-6">
      {/* Header */}
      <div className="flex items-center px-4 py-3 bg-white shadow-sm">
        <Button variant="ghost" size="icon" onClick={() => navigate("/matches")} className="mr-2">
      ...
      {/* Chat messages - further reduced bottom padding */}
      <div className="flex-1 overflow-y-auto p-4 pb-0 space-y-3">
        {messages.map((message, index) => <div key={index} className={`flex ${message.isUser ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] p-3 rounded-lg ${message.isUser ? 'bg-primary text-white rounded-br-none' : 'bg-gray-100 rounded-bl-none'}`}>
              <p>{message.text}</p>
      ...
      <div className="p-2 mt-2 bg-white border-t my-12">
        <div className="flex gap-2">
          ...
          <Button className="bg-primary" onClick={handleSendMessage} disabled={!currentMessage.trim()}>
